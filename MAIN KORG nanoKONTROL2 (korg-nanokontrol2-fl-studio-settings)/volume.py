VOLUME_TABLE = {
	1.0: (5.6,0.0036), 0.98: (5.0,0.0036), 0.943: (4.0,0.0035), 0.907: (3.0,0.0035), 0.87: (2.0,0.0035), 0.835: (1.0,0.0035),
...
def to_db(volume, volume_table=VOLUME_TABLE, end_table=END_TABLE):
	if volume > 1.0: volume = 1.0
	if type(volume) == float:
		if volume >= 0.00102:
			vol = [key for key in volume_table.keys() if key >= volume][-1]
			dist = volume_table[vol][1]
			...
			db = volume_table[vol][0]
			return db-decimals[closest]
		elif volume < 0.00102:
			closest = min(end_table.keys(), key = lambda key: abs(key-volume))
			return end_table[closest]


def to_fl(db, volume_table=VOLUME_TABLE, end_table=END_TABLE):
	if db > 5.6: db = 5.6
	if type(db) == float or type(db) == int:
		if db >= -69.9:
			voldata = [item for item in volume_table.items() if item[1][0] >= db][-1]
			vol = voldata[0] ; voldb = voldata[1][0] ; dist = voldata[1][1]
			return vol - round(voldb-db,1) * 10 * dist
		elif db <= -70.0:
			closest = min(end_table.values(), key = lambda value: abs(value-db))
			end_table_inv = {v: k for k, v in end_table.items()}
			return end_table_inv[closest]
